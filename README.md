...
tests/          # playwright 스위트 20개 + 공용 env.js + run-all.js 러너 (게임 런타임과 무관)
...
| 10 | `js/monsters.js` | `makeMonster` · 엘리트 어픽스 · 팩 어그로 · 매복/소환 · 텔레그래프 강타 |
| 11 | `js/combat.js` | 이동(리더/팔로워) · `updateCombat` · `damageMonster`/`damageMember` · 부활/전멸 · **캐릭터 능력**(`CHAR_ATTACK`/`CHAR_TICK`: 관통·연타·광역·화상·한기·소환·오라·실드·회복병·감속장·변신) · 상태이상 |
| 12 | `js/delve.js` | 아주라이트 광맥 채굴 · 어둠 게이지 · 플레어 |
| 13 | `js/meta.js` | 🌀 주간 모드(ISO 주차 시드 · 변형 룰 8종 · `weeklyMods()`) · 🏆 도전 과제 36종 · 📖 도감 · 메타 세이브 |
| 14 | `js/world.js` | 맵 전환(`enterDungeon`/`enterWeekly`/`descend`/`escapeDungeon`/정산) · 깊이 선택 · 자동 탐험(`autoPlan` 풀 루팅) |
| 15 | `js/endgame.js` | 🪞 환영 모드(안개 확산 · 환영 몬스터 · 보상 게이지 5단계 · 환영 파편) · 🕳️ 우버 보스 2종(4페이즈 AI · 전멸기 · 기믹) · 우버 제단/입장권 |
| 16 | `js/ui.js` | 모달 시스템 + 큐 · 모든 모달(축복/유물/갈림길/상점/**파티 편성**/파티&빌드/**패시브 트리 맵**/설정/기록판/깊이/**도전 과제·도감 탭**) · HUD · 토스트 · 힌트 · 뱃지 |
| 17 | `js/draw.js` | 미니맵 + 렌더링 전부(타일/캐릭터/몬스터/프롭/아이템 빔/이펙트/비네트 · 가이드 방향 화살표) |
| 18 | `js/title.js` | 🎬 타이틀 화면(로고·파티 도트·이어하기 요약) · 첫 런 가이드 코치마크 4단계 · service worker 등록 |
| 19 | `js/main.js` | 입력 · 잡담 타이머 · 메인 루프(전투 속도 2배 dt) · 부트스트랩 · `window.GAME` 디버그 훅 |
