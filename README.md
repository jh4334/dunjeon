...
2. **계단을 밟으면 스테이지 계약** — 후보 지역 **2~3장의 계약 카드** 중 하나를 고릅니다 (보스 층은 분기 없이 고정 진입).
   - 카드 = 바이옴 + 경로 성격(🛡️ 안전 / 💀 위험) + **위험 Modifier 0~3개** + 위험 점수 합 + 보상 배율
   - **보상 배율 = 1 + 0.18 × 위험 점수** — 골드 · XP · 드랍률에 그대로 실립니다 (자세한 내용은 [스테이지 계약](#-스테이지-계약-m8b))
   - 각 카드는 25% 확률로 특수 층이 됩니다 — 💰 **보물방** / ⚔️ **도전방**(3웨이브 아레나) / ⚡ **침공**(90초 인카운터)
   - 일반 층 20% 확률로 🛒 **떠돌이 상인** — 골드로 유물·포션·젬·제작 재화 구매
...
계단의 갈림길이 **지역 후보 2~3장의 계약 카드**로 넓어졌습니다.
카드 하나 = **바이옴 + 경로 성격 + 위험 Modifier 0~3개 + 위험 점수 합 + 보상 배율**.

### ⚠️ 위험 Modifier 12종

| 아이콘 | 이름 | 위험 | 효과 |
|---|---|---|---|
| 🛡️ | 강인함 | 1 | 몬스터 최대 체력 +30% |
| 🔮 | 원소 갑주 | 1 | 몬스터가 특정 원소(화염/냉기/전격) 젬 피해 50% 저항 |
| 🪤 | 가시 지형 | 1 | 함정 2배 |
| 💨 | 신속 | 2 | 몬스터 이동 속도 +25% |
| 👥 | 무리 | 2 | 몬스터 팩 크기 +2 |
| 💜 | 정예 부대 | 2 | 엘리트 등장률 2배 |
| 💔 | 회복 억제 | 2 | 파티 치유량 -40% (포션·샘 포함) |
| ♻️ | 재생 | 2 | 몬스터가 초당 최대 체력 1.5% 회복 |
| 👁 | 어둠 심화 | 2 | 어둠 잠식 속도·피해 +50% |
| ⏳ | 시간 압박 | 2 | 층 제한 3분 — 초과하면 그 층 보상 절반 |
| 🗡️ | 처형자 | 3 | 몬스터 공격이 15% 확률로 피해 2배 |
| 💥 | 폭발적 죽음 | 3 | 몬스터가 죽을 때 주변 1칸 폭발 |

**상충 조합 3쌍**(⏳+💔 / ⏳+♻️ / 🛡️+♻️)은 같은 카드에 함께 붙지 않습니다.
**깊이 스케일** — 얕은 층은 0~1개, 깊이 15+ 는 1~3개가 붙습니다.
특수 층(보물방/도전방/침공)과 보스 층은 기존 규칙 그대로라 Modifier 가 새겨지지 않습니다.

### 💰 보상 공식

```
rewardMult = 1 + 0.18 × 위험 점수 합        (☠️ 타락한 계약은 × 1.5)
위험 점수 = 경로 성격 기본 위험 + 붙은 Modifier 위험
경로 성격 기본 위험: 🛡️ 안전 0 · 💀 위험 2 · 💰 보물방 0 · ⚔️ 도전방 1 · ⚡ 침공 1
```

기존 `wld.riskMult` 를 그대로 승계합니다 — 💀 위험한 경로 단독 = 위험 2점 = **×1.36**.
배율은 **골드 · XP · 드랍률**에 함께 실립니다(드랍률은 과열 방지로 2배에서 자릅니다).

### ⚒️ 계약 제작 (M8a 재화 재사용)

갈림길 모달의 카드에 바로 재화를 쓸 수 있습니다. 결과는 카드의 **시드 + 제작 횟수**로 결정되므로 재현 가능합니다.

| 재화 | 효과 |
|---|---|
| 🔨 재련의 오브 | 위험 Modifier 를 전부 다시 새긴다 |
| ➕ 부여의 오브 | 위험 1개 추가 (보상 배율 상승 · 최대 5개) |
| ☠️ 타락의 오브 | 위험 2개 추가 + 보상 ×1.5 — **이후 변경 불가** |

카드 상태는 층 전환 중에만 존재하므로 저장하지 않습니다(모달을 연 채 새로고침하면 다시 굴립니다).
확정된 계약은 층 생성 시 `wld.contract` 로 옮겨 붙어 층 전체(몬스터 생성·함정·어둠·치유)에 적용됩니다.

### ⚡ 침공 인카운터

갈림길 특수 층 풀(25%)에 들어가는 **90초 아레나**입니다.

- 적이 **연속으로 스폰**됩니다(깊이에 따라 간격 ↓ · 웨이브 ↑ · 동시 생존 20 상한)
- 처치 수 **마일스톤 10 / 25 / 45킬**마다 보상 단계 상승 — 💰 골드 → 🔨 제작 재화 → 🗡️ 희귀 장비
- **엘리트 처치 +8초** (최대 150초)
- **45킬에 침공 지휘관(미니 보스)** 출현 — 나와 있는 동안 타이머는 멈추고, 처치하면 최고 보상(희귀 장비 + 재화 ×2 + 젬)
- 타이머가 끝나면 **그때까지의 단계로 정산**하고 계단이 열립니다
- HUD 에 남은 시간 · 처치 수 · 다음 마일스톤이 표시됩니다

### 📈 밸런스 로그

계약을 끝낼 때마다 `records.contracts` 링버퍼(최근 50개)에
`{ 위험 점수, 클리어 여부, 소요, 깊이 }` 가 쌓이고,
❗ 런 정보에 **"평균 위험도 2.4 · 클리어율 80% (25건)"** 한 줄로 요약됩니다.

**도전 과제 2종** — 📜 위험한 계약(위험 5점 이상 계약 클리어) / ⚡ 침공 격퇴(침공 45킬).
주간 도전 룰과 계약 Modifier 는 **중첩**됩니다(예: 주간 '군단' +2 와 계약 '무리' +2 = 팩 +4).
...
|---|---|---|---|---|
| 일반 | 흰 | 0개 | — | 베이스 그대로 (`검`) |
| 마법 | 파랑 | 1~2개 | 접두 1 + 접미 1 | 접두 + 베이스 (`사나운 단검`) |
| 희귀 | 노랑 | 3~4개 | 접두 2 + 접미 2 | 2어절 고유 이름 + 베이스 (`황혼의 송곳니 (검)`) |
| 고유 | 주황 | 고정 효과 | — | `「망자의 서약」` |
...
- **접두 8 / 접미 8 (M8a)** — 힘 계열(공격력·체력·치명타·치명타 피해·공격 속도·흡혈·치유량·젬 효과)은 **접두**,
  방어·유틸 계열(피해 감소·텔레그래프·어둠 저항·이동 속도·시야·골드·아주라이트·부활)은 **접미**입니다.
  같은 `group` 의 접사는 한 아이템에 두 번 붙지 않습니다.
- **티어 T1~T3 (M8a)** — 기존 굴림 폭(중앙값의 0.8~1.2배)을 정확히 3등분한 것입니다.
  **T3** 는 `ilvl 1+`, **T2** 는 `ilvl 8+`, **T1** 은 `ilvl 16+` 부터 등장하고 상위 티어일수록 희소합니다(55 / 30 / 15).
  T2 의 중앙이 기존 평균과 같으므로 **접사 수치 공식 자체는 그대로**입니다.
- **시드** — 아이템마다 `seed` 와 제작 횟수 `craftN` 을 갖습니다.
  같은 시드 + 같은 제작 이력이면 언제나 같은 결과가 나옵니다(결정성 · 테스트 가능).
...
장비 상세의 `⚒️ 제작` 버튼에서 씁니다. 재화를 고르면 **결과 미리보기**가 뜨고, 확인하면 그 자리에서 적용됩니다.

| 재화 | 효과 | 대상 |
|---|---|---|
| 🔨 재련의 오브 | 고정되지 않은 접사를 **전부 재생성** | 희귀 |
| ➕ 부여의 오브 | 빈 접두/접미 자리에 접사 1개 추가 (**일반→마법→희귀 승급**) | 일반·마법·희귀 |
| ✂️ 절단의 오브 | 고정되지 않은 접사 1개를 무작위 제거 | 접사 보유 |
| 🎲 유동의 오브 | 접사 종류는 유지하고 **수치만** 재롤 | 접사 보유 |
| 🔒 고정의 인장 | 접사 1개를 보호 — 이후 재련·절단·유동에서 보존 (아이템당 1개) | 접사 보유 |
| ☠️ 타락의 오브 | **네 갈래 운명** — 이후 제작 불가 | 전부 |
| 🔥 원소 각인 | 재련하면서 **접두 1개를 원소 계열로 보장** (희소) | 희귀 |
| 🛡️ 수호 각인 | 재련하면서 **접미 1개를 방어 계열로 보장** (희소) | 희귀 |

**☠️ 타락 4갈래 (각 25%)** — ✨ 강력한 암시 옵션 추가 / ⬆️ 접사 1개 티어 상승 / 🩸 부정 접사 추가 / 💥 파괴.
타락한 장비는 **붉은 테두리 + 「타락」 뱃지**가 붙고 더 이상 제작할 수 없습니다.

**획득** — 일반 몹 3.5% / 엘리트 16% / 보스 55% / 상자 12% / 광맥 10% · 떠돌이 상인 재고 · 환영 게이지 단계당 1개.

### 📊 스탯 상세 (M8a)

파티 모달 `🗡️ 장비` 탭의 캐릭터 줄에서 `📊 상세` 를 누르면
**공격력 / 최대 체력 / 치명타 확률 / 이동 속도**를 출처별로 쪼개서 보여 줍니다
(기본 · 영구 강화 · 축복 · 유물 · 패시브 · 키스톤 · 장비 접사 · 일시 버프).
예: `공격력 146 = 기본(Lv.20) 48 × 영구 강화 +24% × 축복 +30% × 패시브 +12% × 키스톤 ×1.40 × 장비 접사 +21%`.
매 프레임이 아니라 **열 때 한 번만** 계산합니다(`statBreakdown(member, stat)`).
...
### 🌳 패시브 트리 (290 노드 · 21 클러스터)

파티 모달의 `패시브` 탭에 **노드 그래프**가 뜹니다(드래그 패닝 · 줌 4단계 · 더블탭 줌 토글 ·
노드 이름 검색 · ◆노터블/★키스톤/🪬소켓 필터 하이라이트 · 클러스터 **미니 오버뷰**).
**인접한 노드만** 찍을 수 있고, **골드 50 × 찍은 노드 수**로 전체 초기화(리스펙)합니다.

**↩️ 오클릭 되돌리기 (M8b)** — 잘못 찍었을 때 리스펙까지 갈 필요가 없습니다.

- **노드 회수** — 빼도 나머지 노드가 시작점과 이어지는 **비절단점**이면 노드 상세의 `↩️ 회수` 로 **포인트 1 환급**(무료).
  절단점이면 버튼이 잠기고 *"이 노드를 회수하면 뒤의 노드가 끊깁니다"* 사유가 뜹니다.
  키스톤·노터블·소켓도 규칙은 같고, 소켓을 회수하면 **끼워 둔 룬이 반환**됩니다.
- **마지막 되돌리기** — 이번 세션에 찍은 순서를 스택으로 들고 있어 `↩️ 마지막 되돌리기` 로 **연속 undo** 가 됩니다.
  이력은 **저장하지 않으므로**(런타임 전용) 재로드 후에는 회수 규칙만 남습니다.

**🎯 최단 경로 자동 할당 (M8b)** — 멀리 있는 노드를 눌러도 *"인접 노드부터 찍으세요"* 로 끝나지 않습니다.
지금 찍은 노드 집합에서 목표까지의 **최단 경로를 BFS로 계산**해 점선으로 하이라이트하고,
상세 패널에 `🎯 N포인트로 여기까지 찍기` 버튼이 뜹니다(포인트가 모자라면 *"3포인트 부족"*).
누르면 경로 전체가 **순서대로** 할당되고 되돌리기 스택에도 그 순서로 쌓입니다.
경로가 여러 개여도 노드 순서를 고정한 결정적 BFS라 **언제나 같은 경로**가 나옵니다.

구조는 세 겹입니다.

| 링 | 구성 | 내용 |
|---|---|---|
| 안쪽 (기존 58) | 소형 40 · 노터블 12 · 키스톤 6 | 공격 🗡️ / 생존 🛡️ / 유틸 🎒 3갈래 + 교차 경로 3줄 |
| 중간 링 9 클러스터 | 소형 81 · 노터블 18 | 광전사 · 저격수 · 결투가 · 수호 · 불굴 · 요새 · 탐광 · 상술 · 현자 |
| 외곽 링 12 클러스터 | 소형 100 · 노터블 20 · 키스톤 9 · 소켓 4 | 화염 · 냉기 · 번개 · 도트 · 소환 · 투사체 · 광역 · 실드 · 흡혈 · 치명타 · 재화 · 어둠·광산 |

외곽 12 클러스터는 **이웃끼리 다리로 이어져 한 바퀴** 돌 수 있어, 전문화를 갈아타는 교차 빌드가 가능합니다.

**★ 키스톤 15** — 전부 이득과 대가가 함께 붙습니다.
...
| 원소 과부하 | 원소(젬) 피해 **+50%** | 치명타가 발생하지 않는다 |
| 불사의 서약 | 초당 최대 체력 **3% 재생** | 모든 치유 무효 |
| 유리 신체 | 실드량 **2배** | 최대 체력 -30% |
| 소환군주 | 미니언 최대 **+2** | 본인이 주는 피해 -40% |
| 혈마법 | 주는 피해 **+35%** | 스킬 시전마다 최대 체력 5% 소모 |
| 광부의 집념 | **어둠 게이지 면역** | 이동 속도 -10% |
| 일격필살 | 치명타 피해 **+100%** | 공격 속도 -30% |
| 부동심 | 멈춰 있으면 받는 피해 **-40%** | 이동 중에는 +15% |
| 결속의 진형 | 파티원 공격 **+30%** | 리더 공격 -30% |

**🪬 룬 소켓 4** — 외곽 클러스터 끝에 있습니다. 런 중에 얻는 **룬 6종**(힘/수호/신속/탐욕/원소/피)을
끼워 노드 효과를 직접 정합니다. 리스펙해도 룬은 사라지지 않고 장착만 풀립니다.

**포인트 공급** — 레벨업 **+1** · 깊이 **5/10/15/20/25/30 최초 도달 +2** · **우버 보스 처치 +3**.

각 갈래의 **초입 5노드**는 구버전 패시브(3갈래 × 5)와 수치가 완전히 같고,
**기존 58노드의 id·효과·연결은 확장 뒤에도 한 글자도 바뀌지 않았습니다** —
구 세이브는 마이그레이션 없이 찍어 둔 노드가 그대로 살아납니다.

### 🪞 환영 모드 (Delirium)

일반 층 **20%** 확률로 **환영의 거울**이 놓입니다. 밟으면 층 전체로 **보라 안개**가 퍼지고:

- 안개에 닿은 몬스터는 **HP·공격력 +40%**
- **환영 몬스터**(보라 잔상)가 계속 추가로 스폰
- **보상 게이지 5단계** — 처치 8/18/30/45/62 마다 확정 보상
  (💰골드 → 💠젬 → 🗡️희귀 장비 → ✨각성젬 → 🔮유물)
- 안개는 시간이 갈수록 짙어져 **초당 피해가 커집니다**(상한 있음) — 오래 버틸수록 위험도 보상도 상승
- **계단을 밟거나 거울로 되돌아오면** 종료 + 정산 → **🔮 환영 파편** 획득

환영 파편은 우버 입장권의 재료입니다.

### 🕳️ 우버 보스

최고 깊이 **15** 를 찍으면 초원의 **우버 제단**이 열립니다.
**환영 파편 30개**로 입장권을 만들고, 전용 아레나에서 우버 보스와 싸웁니다.
**실패해도 잃는 것은 입장권뿐**입니다.

| 보스 | HP | 페이즈 |
|---|---|---|
| 🕳️ **공허의 군주 베이가** | 일반 보스 **×8** | P1 기본 + 공허 레이저 → P2(75%) 아레나 절반 회전 장판 → P3(50%) 분신 3 + 텔레포트 연타 → P4(25%) **전멸기 카운트다운(안전지대 1칸)** + 격노 |
| ⛏️ **타락한 대광부 모르그란** | 일반 보스 **×8** | 어둠 즉시 최대 · 낙석 텔레그래프 연타 → 광차 직선 돌진 → 아주라이트 수정 소환(부수면 잠시 광원) → 전부 동시 + 격노 |

페이즈 전환마다 **무적 + 연출**이 들어갑니다. 모든 공격은 텔레그래프 장판이라
**원샷 상한(최대 HP의 45%) 규칙이 그대로 걸리고, 회피로 공략 가능**합니다.

처치 보상 — ✨**각성젬 확정** · 우버 전용 고유 장비(👑 공허의 왕관 / ⛏️ 모르그란의 곡괭이) ·
🌳 **패시브 +3pt** · 도감/도전 과제 편입. 기록은 주간과 별개로 `records.uber` 에 쌓입니다.
...
- **제작 3** (M8a) — 첫 제작 / 제작 재화 25회 / 타락 10회
- **진행 8** — 누적 킬 1,000 / 누적 골드 10만 / 주간 3주 참여 / 주간 깊이 12 / 상인 20회 / 제단 10회 / 깊이 15 / 레벨 20
- **엔드게임 6** (M7c) — 우버 첫 처치 / 우버 2종 전부 / 환영 5단계 / 환영 파편 100 / 트리 100노드 / 룬 소켓 4칸 채우기
- **계약·침공 2** (M8b) — 📜 위험 5점 이상 계약 클리어 / ⚡ 침공 45킬
...
`state.codex = { mons: {type: kills}, relics: [], gems: [], uniques: [] }` — 총 **117종**(몬스터 48 = 일반 41 + 보스 5 + 우버 2 · 유물 6 · 젬 54 = 스킬 21 + 서포트 12 + 각성 21 · 고유 9 = 기본 7 + 우버 2).
...
| 👤 버튼 | 파티 & 빌드 — 🗡️ 장비 장착/비교/판매, 💠 젬 3슬롯(스킬 1 + 서포트 2 · 각성젬 보라 테두리 · 콤보 미리보기), 🌳 **패시브 트리 맵**(290노드, 줌/검색/필터/미니 오버뷰) |
| ❗ 버튼 | 탭 3개 — **런 정보**(난이도·층·바이옴·처치 수·축복/유물, 초원에서는 최고 기록·영구 강화 요약) / 🏆 **도전 과제 36종** / 📖 **도감** |
| 🗺️ 버튼 | 미니맵 토글 |
...
| `test-m7c` | 환영 모드 · 우버 보스 2종 · 패시브 트리 290노드 · 룬 소켓 |
| `test-m8a` | 접두/접미 · 티어 · 시드 결정성 · 제작 재화 8종 · 타락 4갈래 · 스탯 브레이크다운 |
| `test-m8b` | 스테이지 계약(위험 Modifier 12종 · 보상 공식 · 계약 제작) · 침공 인카운터 · 트리 회수/최단 경로 |
...
- 릴리스 때는 `sw.js` 의 `CACHE` 버전을 올려 캐시를 갱신합니다 (현재 **`dunjeon-v9`**).
...
tests/          # playwright 스위트 21개 + 공용 env.js + run-all.js 러너 (게임 런타임과 무관)
...
| 2 | `js/tree.js` | 패시브 트리 **290노드** — 기존 58노드 + 클러스터 생성기(`CLUSTER_SPECS`/`buildCluster`) · 키스톤 15 · 룬 6종/소켓 4 · `passiveReachable` |
...
| 6 | `js/items.js` | 장비 — 슬롯/레어리티/베이스/접사 16종(**접두 8/접미 8 · 티어 T1~T3 · 시드**)/고유 9종(우버 전용 2종 포함) · **캐릭터 단위 장비 칸** · 인벤토리(상한 40) · 드랍 테이블 · `equipStat` 합산 캐시 |
| 7 | `js/craft.js` | ⚒️ 제작 재화 8종(재련/부여/절단/유동/고정/타락 + 원소·수호 각인) · 제작 규칙/미리보기 · 재화 드랍 · 📊 `statBreakdown` |
| 8 | `js/audio.js` | WebAudio SFX 신스 (오실레이터 + 노이즈 + 엔벨로프) · 🎵 **BGM 3트랙 루프 + 크로스페이드** |
| 9 | `js/mapgen.js` | 월드/타일 · 초원 · `BIOMES` · 레이아웃(rooms/cave/mine/arena/운하/용암) · 연결성 · `genFloor`/`populateFloor` · 시야 |
| 10 | `js/contract.js` | 📜 **스테이지 계약** — 위험 Modifier 12종 · 보상 공식(`contractRewardMult`) · 계약 제작(재련/부여/타락) · 밸런스 로그 · ⚡ **침공 인카운터**(타이머/마일스톤/미니 보스) · 인카운터 공통 헬퍼 |
| 11 | `js/monsters.js` | `makeMonster` · 엘리트 어픽스 · 팩 어그로 · 매복/소환 · 텔레그래프 강타 |
| 12 | `js/combat.js` | 이동(리더/팔로워) · `updateCombat` · `damageMonster`/`damageMember` · 부활/전멸 · **캐릭터 능력**(`CHAR_ATTACK`/`CHAR_TICK`: 관통·연타·광역·화상·한기·소환·오라·실드·회복병·감속장·변신) · 상태이상 |
| 13 | `js/delve.js` | 아주라이트 광맥 채굴 · 어둠 게이지 · 플레어 |
| 14 | `js/meta.js` | 🌀 주간 모드(ISO 주차 시드 · 변형 룰 8종 · `weeklyMods()`) · 🏆 도전 과제 36종 · 📖 도감 · 메타 세이브 |
| 15 | `js/world.js` | 맵 전환(`enterDungeon`/`enterWeekly`/`descend`/`escapeDungeon`/정산) · 깊이 선택 · 자동 탐험(`autoPlan` 풀 루팅) |
| 16 | `js/endgame.js` | 🪞 환영 모드(안개 확산 · 환영 몬스터 · 보상 게이지 5단계 · 환영 파편) · 🕳️ 우버 보스 2종(4페이즈 AI · 전멸기 · 기믹) · 우버 제단/입장권 |
| 17 | `js/ui.js` | 모달 시스템 + 큐 · 모든 모달(축복/유물/갈림길/상점/**파티 편성**/파티&빌드/**패시브 트리 맵**/설정/기록판/깊이/**도전 과제·도감 탭**) · HUD · 토스트 · 힌트 · 뱃지 |
| 18 | `js/draw.js` | 미니맵 + 렌더링 전부(타일/캐릭터/몬스터/프롭/아이템 빔/이펙트/비네트 · 가이드 방향 화살표) |
| 19 | `js/title.js` | 🎬 타이틀 화면(로고·파티 도트·이어하기 요약) · 첫 런 가이드 코치마크 4단계 · service worker 등록 |
| 20 | `js/main.js` | 입력 · 잡담 타이머 · 메인 루프(전투 속도 2배 dt) · 부트스트랩 · `window.GAME` 디버그 훅 |
...
- `craft.js` 는 로드 시점에 재화 표만 만들지만 접사/티어 테이블(items.js)에 기대므로 → **items.js 가 먼저**
- `ui.js` 가 로드 중 `escapeDungeon`(world.js)을 버튼에 바인딩 → **world.js 가 먼저**
- `endgame.js` 는 world.js 의 `transition`/`showRunSummary` 를 런타임에만 쓰므로 world.js 뒤 · ui.js 앞이면 충분
- `contract.js` 는 `PATH_KINDS`(mapgen.js)와 재화(craft.js)를 런타임에만 쓰지만, mapgen 의 `genFloor` 가 계약을 정규화하므로 **mapgen.js 뒤 · monsters.js 앞**에 둡니다
